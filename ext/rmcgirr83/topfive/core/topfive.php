		...
		*/
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$tpl_ary = array(
				...
				'USERNAME_FULL'		=> ($is_guest || !$this->auth->acl_get('u_viewprofile')) ? get_username_string('no_profile', $row['user_id'], $row['username'], $row['user_colour'], $row['topic_last_poster_name']) : get_username_string('full', $row['user_id'], $row['username'], $row['user_colour']),
