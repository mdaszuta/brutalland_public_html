	...
	public function toptopics($tpl_loopname = 'top_five_topic')
	{
		...
		*/
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$view_topic_url = !empty($post_unread) ? append_sid("{$this->root_path}viewtopic.$this->php_ext", 't=' . $topic_id . '&amp;view=unread') . '#unread' : append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $row['forum_id'] . '&amp;p=' . $row['topic_last_post_id'] . '#p' . $row['topic_last_post_id']);
			$view_topic_url_last_post = append_sid("{$this->root_path}viewtopic.$this->php_ext", 'f=' . $row['forum_id'] . '&amp;p=' . $row['topic_last_post_id'] . '#p' . $row['topic_last_post_id']);
			...
			**/
			...
			$tpl_ary = [
				...
				'U_TOPIC_LAST_POST'	=> $view_topic_url_last_post,
